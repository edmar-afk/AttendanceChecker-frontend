...
export default function FaceRecognition() {
  ...
  const uploadTimeIn = async () => {
    ...
    try {
      const { data } = await api.post(`/api/facerecognition-timein/${attendanceId}/${matchedUser.user_id}/`, {});
      // ${attendanceId}
      // ${matchedUser.name}
      ...
      {matchedUser && (
        <div className="mt-6 p-4 bg-blue-100 rounded-lg border border-blue-200 text-center">
          <h3 className="text-blue-800 font-semibold">
            Face Matched! User ID: {matchedUser.user_id} - {matchedUser.user_id}
          </h3>
